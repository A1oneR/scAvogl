...
public void OnPluginStart()
{
	...
	h_AfkWarnKickTime = CreateConVar("l4d_specafk_warnkicktime", "20", "Warn time before kick (while already on spec)", FCVAR_SPONLY|FCVAR_NOTIFY, false, 0.0, false, 0.0);
	h_AfkKickTime = CreateConVar("l4d_specafk_kicktime", "20", "time before kick (while already on spec after warn)", FCVAR_SPONLY|FCVAR_NOTIFY, false, 0.0, false, 0.0);
	...
	for (i=1;i<=MaxClients;i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i))
		{
						...
						if (L4D2_GetInfectedAttacker(i) == -1)
						{
							...
							else // player warn timeout reached ...
							{
								...
								if (afkPlayerTimeLeftAction[i] <= 0)
								{
									...
									{
										PrintHintText(i, "[AFK] 偵測閒置! 一旦有人離開安全區域你將強制旁觀.");
									}
								}
			...
			else if (afkKickEnabled)  // if player is on spectators and kick on spectators is enabled ...
			{
				...
				if ((survReadyCount + infReadyCount) < survLimit + zombLimit) // In game players < ShouldBe
				{
